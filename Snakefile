...
rule plot_link:
    ...
    run:
        ...
        if df_filtered.empty:
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.set_title(f"Time vs Precision - Function: {function_name}")
            ax.set_xlabel("Time (seconds)")
            ax.set_ylabel("Precision (1/StdDev^2)")
            ax.set_xscale("log")
            ax.grid(True)
            for aggregator, sub_group in df_filtered.groupby("aggregator"):
                sub_group = sub_group.sort_values("time")
                sns.lineplot(ax=ax, x="time", y="precision", data=sub_group, marker="o", label=f"{aggregator}")
